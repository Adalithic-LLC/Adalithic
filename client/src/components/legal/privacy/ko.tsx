...
export function PrivacyKo() {
  return (
    <div className="space-y-4 text-sm text-gray-600 leading-relaxed">
      ...
      <p className="font-medium">G. 타이핑 및 Glide 품질에 대한 익명 진단 정보</p>
      <p>
        자동 수정과 Glide(밀어서 입력) 타이핑을 측정하고 개선하기 위해 Arcatext는 이러한 기능이 얼마나 잘 작동하는지에 대한 익명의 집계된 진단 정보를 수집합니다. 이 정보는 설치별 임의 식별자와 연결되며, 귀하의 계정, 이름, 이메일 또는 기기 신원과는 연결되지 않습니다.
      </p>
      <p>자동 수정의 경우 다음이 포함됩니다.</p>
      <ul className="list-disc pl-6 space-y-1">
        <li>자동 수정이 유지되거나 취소된 빈도</li>
        <li>어떤 글자가 어떤 다른 글자로 바뀌었는지에 대한 집계 수치</li>
        <li>키 입력 횟수</li>
        <li>익명의 탭 정확도 측정값(탭이 키 중앙에서 얼마나 가까이 입력되는지)</li>
        <li>앱의 현재 수정 설정</li>
      </ul>
      <p>Glide 타이핑의 경우 다음이 포함됩니다.</p>
      <ul className="list-disc pl-6 space-y-1">
        <li>몇 번의 밀어서 입력이 단어를 만들어 냈는지</li>
        <li>다른 추천 단어를 선택하거나 결과를 삭제한 빈도</li>
        <li>의도한 단어가 추천 목록에서 몇 번째에 나타났는지</li>
        <li>밀어서 입력한 궤적에 대한 익명의 측정값(일반적인 속도, 부드러움, 키 중앙에서 얼마나 떨어져 지나갔는지 등)</li>
        <li>앱의 현재 Glide 설정</li>
      </ul>
      <p>두 가지 모두 앱 버전, 운영 체제 버전 및 사용 중인 키보드 언어를 포함합니다.</p>
      <p>어느 쪽도 귀하가 입력한 단어, 글자 또는 메시지를 포함하지 않습니다. 입력된 내용은 전혀 수집되지 않습니다. 집계된 글자 변경 및 궤적 측정값으로는 귀하가 작성한 내용을 복원할 수 없습니다. 이 정보는 오직 타이핑 품질을 측정하고 개선하는 데에만 사용됩니다. 판매되지 않으며, 광고 목적으로 공유되지 않고, 다른 회사의 앱이나 웹사이트에서 귀하를 추적하는 데 사용되지 않습니다.</p>
      <p>Arcatext 기본 앱의 Reword 데이터 저장 설정에서 언제든지 이 기능을 끌 수 있습니다. 기본적으로 켜져 있으며, 이 하나의 설정으로 자동 수정과 Glide 진단 정보가 모두 적용됩니다.</p>
      <p className="font-medium">H. 우리가 의도적으로 수집하지 않는 정보</p>
